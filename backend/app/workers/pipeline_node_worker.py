"""Pipeline 노드 실행 worker (Phase 3.2.1 chassis, 3.2.2 노드 dispatcher).

`process_node_event(node_run_id, run_date_iso)` actor 가 enqueue 되면 노드 1개를
실행하고 `complete_node` 로 결과를 보고. node_type 별 실제 로직은
`app/domain/nodes/<type>.run(context, config)` 가 담당.
...
from app.domain.nodes import NodeContext, NodeError, NodeOutput, get_node_runner
...
def _execute_node(session: Session, node_run: NodeRun) -> NodeOutput:
    """노드 타입별 실제 작업 디스패치 — `app/domain/nodes/*.run` 로 위임.

    Phase 3.2.2 — NOOP 외 6 노드 (SOURCE_API/SQL_TRANSFORM/DEDUP/DQ_CHECK/
    LOAD_MASTER/NOTIFY) 모두 지원.
    """
    runner = get_node_runner(node_run.node_type)
    nd = session.get(NodeDefinition, node_run.node_definition_id)
    config: dict[str, Any] = {} if nd is None else dict(nd.config_json or {})
    context = NodeContext(
        session=session,
        pipeline_run_id=node_run.pipeline_run_id,
        node_run_id=node_run.node_run_id,
        node_key=node_run.node_key,
        user_id=None,  # 워커에서 사용자 컨텍스트 미주입 (Phase 3.2.x 후속)
    )
    return runner.run(context, config)
...
def process_node_event(
    ...
    def _handler(session: object) -> dict[str, Any]:
        ...
        try:
            node_output = _execute_node(session, nr)  # type: ignore[arg-type]
        except NodeError as exc:
            # NodeError 는 비즈니스 실패 — 노드 FAILED 종결, downstream SKIPPED.
            ...
            # 인프라 예외 — actor retry / DLQ 회부. consume_idempotent 가 rollback.
            raise
        ...
        if node_output.status == "failed":
            completion = complete_node(
                ...
                output_json=node_output.payload,
                pubsub=pubsub,
            )
            return {
                "status": "FAILED",
        ...
        return {
            "status": "SUCCESS",
            "row_count": node_output.row_count,
            "pipeline_status": completion.pipeline_status,
            "next_ready": list(completion.next_ready_node_run_ids),
        }
