...
| # | 단계 | 메뉴 | 산출물 |
|---|---|---|---|
| 1 | Source 등록 | **Source / API Connector** 또는 **Inbound Channel** | 외부 호출 정의 / push 수신 채널 |
| 2 | Field Mapping | **Field Mapping Designer** | 응답 path → mart 컬럼 매핑 + 변환 함수 |
| 3 | Mart 정의 | **Mart Workbench** | 적재 테이블 DDL + load_policy |
| 4 | Canvas 작성 | **ETL Canvas V2** | 자산 박스를 끌어 워크플로 조립 (20 노드) |
| 5 | 실행 + 모니터링 | **Pipeline Runs** + **Operations Dashboard** | run 이력 + SLA / 알람 / 재실행 |

옆길로 **Quality Workbench (DQ rule)**, **Transform Designer (SQL/HTTP/Function)** 등이 4번 Canvas 에서 사용할 추가 자산을 만드는 화면.
...
### 20. Mock API (`/v2/mock-api`) — Phase 8.6 신규

**기능**
- *우리 시스템 안에서 외부 API 흉내내는* endpoint 등록
- 5종 응답 포맷 (json/xml/csv/tsv/text) + status_code + delay_ms (지연 시뮬)

**목적** — 외부 API 의존 없이 시스템 자체로 검증 가능하게.

**사용법**
1. [Mock 등록] → code · 응답 본문 (sample 자동 채움) → 저장
2. serve URL 복사 (`http://localhost:8000/v2/mock-api/serve/{code}`)
3. Source/API Connector 폼에 그 URL 붙여넣기
4. 같은 시스템의 connector 가 외부 API 처럼 mock 호출

---

### 21. Dead Letters (`/dead-letters`)
...
[Admin]
Mock API (테스트) / Dead Letters / Users / API Keys / Security Events / Partition Archive
```
