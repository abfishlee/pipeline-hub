...
- [x] `wf.workflow_definition.schedule_cron` + `schedule_enabled` 편집 UI ✅ 2026-04-26
  - migration `0020_workflow_schedule.py` (schedule_cron TEXT NULL + schedule_enabled BOOL DEFAULT FALSE + active partial index)
  - PATCH `/v1/pipelines/{id}/schedule` — cron 5필드 검증 (croniter), cron=null 이면 enabled 자동 false
  - PipelineDesigner 툴바 (PUBLISHED 워크플로 한정): cron 입력 + 활성 토글 + "스케줄 저장" 버튼 + 현재 cron/상태 표시
  - 실제 cron 트리거는 Phase 4 Airflow 통합 시 수신자가 활용 — 본 phase 는 메타 + UI 까지
- [x] Backfill: 시작/종료 기간 입력 → 일자별 run 생성 ✅ 2026-04-26
  - `app/domain/pipeline_schedule.backfill` — 같은 (workflow, run_date) 가 있으면 기존 ID 반환 (멱등). 366일 한도. cycle 검증.
  - POST `/v1/pipelines/{id}/backfill` (PUBLISHED 만, 202)
  - Designer 의 Backfill 토글 UI (날짜 2개 + 실행 버튼)
- [x] 실행 이력 검색 (status, 기간, workflow) ✅ 2026-04-26
  - `app/domain/pipeline_schedule.search_runs`
  - GET `/v1/pipelines/runs?workflow_id&status&from&to&limit&offset`
  - PipelineRunsList 의 select 3개 (workflow / status / 기간) + 필터 초기화 버튼 — 기존 비활성 placeholder 가 정식 동작.
- [x] 수동 재실행 / 특정 노드부터 재실행 ✅ 2026-04-26
  - `app/domain/pipeline_schedule.restart_run` (from_node_key 의 ancestors 는 SUCCESS 시드, target 은 READY, 후손은 PENDING). entry node ready 정책은 start_pipeline_run 동등.
  - POST `/v1/pipelines/runs/{id}/restart` (202, ADMIN/APPROVER, broker 미가동에도 200 정책)
  - PipelineRunDetail: "처음부터 재실행" 헤더 버튼 + 각 노드 row 의 "이 노드부터" 버튼 (FAILED/SUCCESS 노드만)
- [x] 통합 테스트 `tests/integration/test_pipeline_schedule.py` ✅ 2026-04-26
  - cron 정상/4필드 거부/잘못된 분 거부, cron=null 시 enabled 자동 false
  - backfill 3일치 멱등, start>end 거부, DRAFT 거부 (409)
  - search runs (workflow / status / 기간) 필터
  - restart 전체 (entry READY) / from_node_key="B" (A SUCCESS 시드, B READY, C PENDING) / unknown node_key 거부

**다음 sub-phase 로 이연**
- 실제 cron 트리거 — Phase 4 Airflow DAG 통합 (`schedule_enabled = TRUE` 인 PUBLISHED 워크플로를 매분 polling 하거나 Airflow sensor 로 발화)
- ancestor 노드 output_json 재현 — 현재 restart from_node_key 는 SUCCESS 만 시드, 실제 lineage 데이터는 노드별 핸들링 (Phase 3.2.5+ lineage 결합 후)

### 3.2.8 문서/템플릿 [W9~W10]

- [x] `docs/pipelines/` 에 샘플 파이프라인 YAML 3종 ✅ 2026-04-26
  - `retail_api_price.yaml` — EMART_API SOURCE_API → DQ_CHECK x2 → SQL_TRANSFORM → DEDUP → LOAD_MASTER → NOTIFY (7 nodes / 7 edges, cron `0 5 * * *`)
  - `receipt_ocr.yaml` — RECEIPT_OCR SOURCE_API → DQ x2 → 마스킹 SQL → DEDUP → outlier DQ → LOAD_MASTER → NOTIFY (7 nodes / 7 edges, cron `0 2 * * *`)
  - `online_crawl.yaml` — COUPANG_CRAWL SOURCE_API → DQ x2 → 단위 정규화 → outlier DQ (BLOCK) → DEDUP → LOAD_MASTER → NOTIFY (7 nodes / 6 edges, cron `0 * * * *`)
- [x] `scripts/seed_default_pipelines.py` ✅ 2026-04-26
  - `idempotent_load_yaml(session, [(path, spec), ...])` — 같은 `name` 이 이미 있으면 SKIPPED, 아니면 CREATED. dry-run 모드.
  - schedule_cron 은 시드되지만 schedule_enabled 는 항상 False (운영자가 UI 에서 켬).
- [x] `docs/sql_templates/` 에 SQL 템플릿 12개 + meta.yaml ✅ 2026-04-26
  - 카테고리: 표준화 검증 / 중복 진단 / 결측 비율 / 이상치 (MAD) / 가격 분포 / 신선도 (source 별·전체) / 카디널리티 / mart-stg 정합성 / 고아 row 진단 / 이상 변동 / 배포 이력
  - 모두 sqlglot validate 통과 — 실 테이블(`stg.price_observation`, `mart.price_fact`, `wf.pipeline_release`) 사용
  - `sqlglot_validator._cte_names` 추가 — `WITH foo AS (...)` 의 alias 가 unqualified table 로 잘못 잡히던 버그 수정 (3.2.4 의 검증 정책 보강)
- [x] `scripts/seed_sql_templates.py` ✅ 2026-04-26
  - `idempotent_load_templates` — 같은 `sql_query.name` 있으면 SKIPPED. description 은 `[<카테고리>] <설명>` prefix.
  - 시드 시점에 sqlglot validate 한 번 더 실행 → REJECTED 면 row 미생성 + 경고.
  - sql_query_version v1 DRAFT + current_version_id 동기화.
- [x] 통합 테스트 `tests/integration/test_seed_pipelines_and_sql_templates.py` ✅ 2026-04-26
  - 3 YAML 파싱 → 사용된 6 node_type 모두 등장
  - pipelines 시드 멱등성 (CREATED → SKIPPED 같은 ID)
  - retail_api_price__emart 노드 7개 + 엣지 7개 정확 적재 + cron/enabled 검증
  - 12 템플릿 meta 파싱
  - sql_templates 시드 멱등성 + DRAFT v1 + current_version_id + description prefix + referenced_tables
...
| 항목 | 목표 | 측정 스크립트 | 마지막 측정 | 결과 | 통과 |
|---|---|---|---|---|---|
| Designer 렌더 (100 노드) | ≤ 1000 ms | [`frontend/tests/perf/test_designer_render.spec.ts`](../../frontend/tests/perf/test_designer_render.spec.ts) | _pending live run_ | — | ⏳ |
| SSE publish→DOM 반영 | ≤ 500 ms | [`backend/tests/perf/test_sse_latency.py`](../../backend/tests/perf/test_sse_latency.py) | _pending live run_ | — | ⏳ |
| sandbox 100회 read-only 격리 | row count 불변 + 평균 ≤ 500 ms | [`backend/tests/perf/test_sandbox_isolation.py`](../../backend/tests/perf/test_sandbox_isolation.py) | _pending live run_ | — | ⏳ |
| sqlglot 금지 연산 차단 | 100% (DROP/DELETE/COPY/pg_read_*/pg_catalog/disallowed schema) | [`backend/tests/integration/test_sql_studio.py`](../../backend/tests/integration/test_sql_studio.py) | 2026-04-25 | 12 / 12 통과 | ✅ |
| lineage 자동 추출 | ≥ 95% (sqlglot referenced_tables) | [`backend/tests/integration/test_seed_pipelines_and_sql_templates.py`](../../backend/tests/integration/test_seed_pipelines_and_sql_templates.py) | 2026-04-26 | 12 SQL 모두 referenced_tables 정확 추출 | ✅ |

### 측정 절차

`PERF=1` 환경변수로 perf 테스트 활성화 + 인프라(실 PG / Redis / 백엔드 인스턴스) 가동 후
`backend/tests/perf/README.md` 참조. 결과는 본 표의 *마지막 측정 / 결과 / 통과* 컬럼에
직접 갱신.

미달 시 GH issue 를 만들고 표의 *항목* 칸에 링크. 본 phase 종료 시점에서 ⏳ 인 항목은
실 인프라(NCP) 가동 직후 1회 측정해 baseline 확정 — 이는 Phase 4 진입 게이트의 일부.

---

## 3.5 보안 체크

- [x] SQL sandbox 는 *임시 스키마 대신 read-only TX + ROLLBACK* (ADR-0008) — Phase 4
  마이그 트리거 정의됨. ✅ 2026-04-26
- [x] sandbox SQL 실행은 `SET LOCAL statement_timeout = 30000` 강제. ✅ 2026-04-26
- [x] sandbox connection 은 `SET LOCAL transaction_read_only = ON` — sqlglot 화이트리스트
  + read-only TX + 명시 ROLLBACK 의 3중 방어. ✅ 2026-04-26
- [x] 모든 SQL 실행 (VALIDATE / PREVIEW / EXPLAIN) 은 `audit.sql_execution_log` 원문 +
  sql_hash + 결과 status 기록 (BLOCKED/SUCCESS/FAILED). ✅ 2026-04-26
- [x] API Approver ≠ Submitter — `studio.approve_version` 에서 self-approval 차단,
  통합 테스트로 검증 (`test_approve_self_submitted_blocked`). ✅ 2026-04-25
