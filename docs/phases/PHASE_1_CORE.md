...
- [ ] 최상위 구조 생성: `backend/`, `frontend/`, `migrations/`, `infra/`, `scripts/`, `tests/`
- [ ] `README.md` 최소 (빌드/기동 방법만)
- [ ] `.env.example` 작성 (아래 1.4 참고)
- [ ] `infra/docker-compose.yml` 에 postgres:16, redis:7, minio, mailhog(선택), adminer(선택) 정의
- [ ] `make dev-up` / `make dev-down` 스크립트 (또는 `pnpm run` + `uv run` 스크립트)
- [ ] CI 기본 (GitHub Actions): lint + test + typecheck
