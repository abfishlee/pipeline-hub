...
### 1.2.9 Frontend 스켈레톤 [W4~W6] ✅ 2026-04-25

- [x] `frontend/` Vite + React 18 + TypeScript + Tailwind 3 + pnpm
- [x] shadcn-style UI (Button/Input/Card/Table/Badge/Dialog=Radix/Toast=sonner) — 직접 작성
- [x] 레이아웃 — 사이드바 5개 메뉴(lucide 아이콘) + 헤더 + 사용자 정보 + 로그아웃 버튼
- [x] 인증 — `useAuthStore`(Zustand+sessionStorage) + 부팅 시 `/v1/auth/me` 자동 + 401 자동 logout
- [x] 라우팅 (React Router v6):
  - [x] `/login` — 인증 시 `/` 리다이렉트
  - [x] `/` 대시보드 — 활성 소스 / 오늘 작업/성공/실패 카운터 + 최근 실패 5건
  - [x] `/sources` — 목록 + 생성/수정 Dialog + soft delete (ADMIN 가드)
  - [x] `/jobs` — source_id/status/job_type 필터 + 페이지네이션
  - [x] `/raw-objects` — 목록 + 상세 Dialog (inline payload OR presigned download URL)
  - [x] `/users` — ADMIN-only, 사용자 목록 + 생성 (역할 토글)
  - [x] 미인증 → `/login` / `/users` 는 비-ADMIN 시 `/`
- [x] API 클라이언트 — fetch wrapper + JWT + 401 자동 처리 + ApiError 클래스
- [x] TanStack Query hooks 10종 + sonner Toast (에러/성공)
- [x] **Dockerfile** (multi-stage Node 빌드 → Nginx 정적, non-root, /healthz, NKS Ready 8계명)
- [x] **`nginx/default.conf`** — SPA fallback + `/v1/` 백엔드 프록시 + 60MB 파일 + Request-ID 전파
- [x] BACKEND_URL 환경변수로 백엔드 주소 override
- [x] **로컬 검증**: `pnpm tsc -b` 통과 / `pnpm build` 성공 (333KB JS gz 105KB) / `pnpm dev` → 백엔드 8001 프록시 → `/v1/auth/login` 200 + JWT + `/v1/auth/me` `roles=['ADMIN']` 확인

### 1.2.10 관제 기초 [W6]

- [x] `prometheus_client` 통합, `/metrics` 엔드포인트 ✅ 2026-04-25 (`app/core/metrics.py`, `HttpMetricsMiddleware`, `/metrics` 인증 없음 — 내부 scrape 전용)
- [x] 커스텀 메트릭: HTTP req/duration, DB pool, ingest counter/dedup/bytes ✅ 2026-04-25 (event_outbox backlog 는 Phase 2 outbox publisher 도입 시 추가)
- [x] `infra/` Prometheus + Grafana compose 추가 ✅ 2026-04-25 (`prometheus:v3.0.1`, `grafana:11.3.0`, `host.docker.internal:host-gateway`)
- [x] Grafana dashboard JSON 1장 (`infra/grafana/dashboards/core.json`) ✅ 2026-04-25 — 9 패널 (수집 QPS, dedup율, 24h 누적, p95, 5xx, source×kind, outbox placeholder, DB pool)
- [x] NCP 운영 이식성을 위해 모든 메트릭은 `scrape_configs`로 노출 ✅ 2026-04-25 (path 라벨은 라우트 템플릿 — cardinality 정책 `docs/ops/MONITORING.md` 1.3)
- [x] `audit.access_log` 미들웨어 — 모든 `/v1/*` 응답 후 fire-and-forget INSERT ✅ 2026-04-25 (`app/core/access_log.py`)
- [x] `docs/ops/MONITORING.md` — 운영팀 9월 합류 전 가시성 가이드 ✅ 2026-04-25

### 1.2.11 문서화 & 마무리 [W7~W8]

- [x] `docs/phases/PHASE_1_CORE.md` 1.2.1~1.2.11 체크박스 완료 기록 ✅ 2026-04-25
- [x] `docs/phases/CURRENT.md` 를 `PHASE_2`로 전환 ✅ 2026-04-25 (Phase 1 DoD ✅, Phase 2 진입 조건/대상 모듈 명시)
- [x] ADR 3건 정리 ✅ 2026-04-25
  - `docs/adr/0001-postgres-driver-dual-asyncpg-psycopg.md` — Phase 1.2.3
  - `docs/adr/0002-object-storage-client-choice.md` — Phase 1.2.6 (boto3 + asyncio.to_thread)
  - `docs/adr/0003-outbox-content-hash-dedup.md` — Phase 1.2.7 (Outbox + 분리된 content_hash_index)
- [x] `README.md` 기동 절차 + Grafana(`http://localhost:3000`) 안내 + 스크린샷 placeholder ✅ 2026-04-25
- [x] `scripts/seed_admin.py` — 부트스트랩 ADMIN 사용자 1회 생성 (POST /v1/users 가 ADMIN 가드라 첫 사용자는 별도 채널 필요) ✅ 2026-04-25
- [x] `docs/dev/PHASE_1_E2E.md` — 1.3 의 10단계를 운영팀 9월 합류 시 그대로 따라할 수 있는 curl/SQL 시나리오로 분리 ✅ 2026-04-25

---

## 1.3 샘플 시나리오 (E2E 확인)

Phase 1 종료 시 반드시 통과 — **실행 가능한 curl/SQL 시나리오는 [`docs/dev/PHASE_1_E2E.md`](../dev/PHASE_1_E2E.md) 로 분리**. 운영팀 9월 합류 시 가장 먼저 따라할 문서. 아래는 단계 요약:
